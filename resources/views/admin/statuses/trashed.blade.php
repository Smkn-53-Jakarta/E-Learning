                            ...
                            <div class="table-responsive fixed-actions-table">
                                <table class="table align-middle table-row-dashed fs-6 gy-5 mb-0">
                                    ...
                                    <tbody class="fw-semibold text-gray-600">
                                        @foreach ($statuses as $statusf)
                                            <tr>
                                                <td>{{ $loop->iteration }}</td>
                                                <td>{{ $statusf->name }}</td>
                                                <td>{{ $statusf->deleted_at->diffForHumans() }}</td>
                                                <td>
                                                    <a href="#"
                                                    ...
                                                    <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-125px py-4"
                                                        ...
                                                        @can('statuses.restore')
                                                            <div class="menu-item px-3">
                                                                <form action="{{ route('statuses.restore', $statusf->id) }}"
                                                                    method="post">
                                                                    @csrf
                                                                    <a href="#" class="menu-link px-3"
                                                                        onclick="confirmPopup(this, 'Akan mengembalikan status ini')">Pulihkan</a>
                                                                </form>
                                                            </div>
                                                        @endcan
                                                        @can('statuses.delete')
                                                            <div class="menu-item px-3">
                                                                <form
                                                                    action="{{ route('statuses.force-delete', $statusf->id) }}"
                                                                    method="post">
                                                                    @csrf
                                                                    <a href="#" class="menu-link px-3"
                                                                        onclick="confirmPopup(this, 'Akan menghapus permanen status ini')">Hapus
                                                                        Permanen</a>
                                                                </form>
                                                            </div>
                                                        @endcan
